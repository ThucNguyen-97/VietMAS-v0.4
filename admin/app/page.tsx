...
export default function Dashboard() {
  ...
  const partnerHistoryPanel = <section className="section"><div className="section-title-row"><div><h2>Lịch sử chỉnh sửa</h2></div>{canEdit && partnerHistory.length > 0 && <button onClick={clearPartnerHistory}>Xóa lịch sử</button>}</div><table><thead><tr><th>Thời gian</th><th>Thao tác</th><th>Người chỉnh sửa</th><th>Đối tác</th><th>MST</th><th>Trạng thái</th></tr></thead><tbody>{partnerHistory.map((entry) => <tr key={entry.id}><td>{new Date(entry.changed_at).toLocaleString("vi-VN")}</td><td>{entry.action === "created" ? "Khởi tạo" : entry.action === "deleted" ? "Ngừng hoạt động" : "Cập nhật"}</td><td>#{entry.changed_by_id}</td><td>{entry.snapshot.legal_name}</td><td>{entry.snapshot.tax_code}</td><td>{entry.snapshot.status === "active" ? "Đang hoạt động" : "Ngừng hoạt động"}</td></tr>)}</tbody></table>{!partnerHistory.length && <p className="muted">Chưa có lịch sử chỉnh sửa.</p>}</section>;
  return <main className={`${activeTab === "settings" && settingsTab === "partners" ? "settings-partners" : ""} ${showPartnerForm ? "partner-form-open" : ""}`}>
    ...
    {activeTab === "settings" && settingsTab === "partners" && <div className="settings-tabs partner-tabs"><button className={partnerTab === "list" ? "active" : ""} onClick={() => setPartnerTab("list")}>Danh sách đối tác</button><button className={partnerTab === "history" ? "active" : ""} onClick={() => setPartnerTab("history")}>Lịch sử chỉnh sửa</button></div>}
    {activeTab === "settings" && settingsTab === "partners" && partnerTab === "list" && !editingPartner && <div className="partner-add-bar"><h2>Đối tác</h2>{canEdit && <button onClick={() => setShowPartnerForm((open) => !open)}>{showPartnerForm ? "− Đóng form" : "+ Thêm đối tác"}</button>}</div>}
